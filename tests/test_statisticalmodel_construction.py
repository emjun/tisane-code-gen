...
class ConstructStatisticalModelTest(unittest.TestCase):
    def test_construct_main_only(self): 
        ...
        self.assertEquals(sm.dependent_variable, "PINCP")
        ...
        self.assertEquals(len(sm.interaction_effects), 0)
        self.assertIsInstance(sm.random_effects, set)
        self.assertEquals(len(sm.random_effects), 0)
        self.assertEquals(sm.family_function, "Gaussian")
        self.assertEquals(sm.link_function, "identity")